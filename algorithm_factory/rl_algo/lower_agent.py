    ...
    def rollout_thread(self):
        ...
        with torch.no_grad():
            for i in range(len(self.test_solus)):
                ...
                for step in range(solu.init_env.J_num_all):
                    cur_mission = solu.iter_env.mission_list[step]
                    self.thread_makespan = [float("inf") for _ in range(solu.init_env.ls_num)]
                    threads = [threading.Thread(target=self.parallel_solve, args=(j, solu, step)) for j in
                               range(solu.init_env.ls_num)]
                    for thread in threads: thread.start()
                    for thread in threads: thread.join()
                    min_makespan = min(self.thread_makespan)
                    min_pos = self.thread_makespan.index(min_makespan)
                    makespan = solu.step_v2(min_pos, cur_mission, step)
                    e_t_g = time.time()
                makespan_forall.append(makespan)
                time_forall.append(e_t_g - s_t_g)
